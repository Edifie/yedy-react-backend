...
const TemplateMS = require("../models/templateMusicStore");
...
const createAd = (req, res, next) => {
  ...
  const { pageId, price, category, subCategory, brand, adTitle, description } =
    req.body;
  if (!files) {
  ...
  let newUpload = new TemplateMS(finalImg);
  newUpload
    .save()
    .then(() => {
      res
...
const getTemplateById = async (req, res, next) => {
  ...
  try {
    template = await TemplateMS.findById(templateId);
  } catch (err) {
...
const updateTemplateById = async (req, res, next) => {
  ...
  const files = req.files;
  const { price, category, subCategory, brand, adTitle, description } =
    req.body;
  ...
  TemplateMS.findByIdAndUpdate(templateId, updates)
    .then(() => {
